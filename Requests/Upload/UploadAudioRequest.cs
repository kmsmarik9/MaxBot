...
{
    public partial class UploadAudioRequest : UploadDataBase<UploadAudioResponse>
    {
...
namespace KmsDev.MaxBot.Full
{
    public static partial class MaxBotExtensions
    {
        ...
        /// - Union1: Exception
        /// </returns>
        public static async Task<OneOf<(string Url, string Token, int Retval), MaxBotRequestException>> UploadAudioAsync(this MaxBotClientApiContainer.UploadSection uploadSection, Stream stream, string? fileName = null, Action<UploadAudioRequest.ResilienceDefaultSettings>? resilienceSettingsAction = default, CancellationToken cancellationToken = default)
        {
            try
            {
                var uploadFileUrlResponse = await uploadSection.GetUploadFileUrlAsync(MaxBotUploadType.Audio, cancellationToken: cancellationToken);
                ...
                var requestResilienceSettings = new UploadAudioRequest.ResilienceDefaultSettings();
